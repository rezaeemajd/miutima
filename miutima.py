...
def estimate(url: str, media: str, height: int | None) -> str:
    options = common()
    options.update(skip_download=True, noplaylist=True)
    options["format"] = "bestaudio/best" if media == "mp3" else f"bestvideo[height<={height or 720}]+bestaudio/best[height<={height or 720}]"
    try:
...
def download(url: str, media: str, height: int | None = None, bitrate: str = "192") -> bool:
    ...
    options = common()
    options.update(outtmpl=str(output_dir / "%(title)s.%(ext)s"), progress_hooks=[hook], ffmpeg_location=ffmpeg(), noplaylist=True, writethumbnail=bool(config["embed_thumbnail"]), addmetadata=bool(config["embed_metadata"]))
    if config["subtitle"]:
        options.update(writesubtitles=True, subtitleslangs=[str(config["subtitle_language"])], subtitlesformat="srt/vtt/best")
    if media == "mp3":
        ...
        options.update(format=f"bestvideo[height<={selected_height}]+bestaudio/best[height<={selected_height}]", merge_output_format="mp4")
...
def history_menu() -> None:
    ...
    for number, item in enumerate(items[:20], 1):
        table.add_row(str(number), str(item.get("title", ""))[:60], str(item.get("type", "")), str(item.get("date", ""))[:19])
    console.print(table)
...
def search_menu() -> None:
    ...
    try:
        ...
        console.print("[dim]If you are using WSL and see 'Connection refused', miutima will retry without the proxy. Check WSL Internet/DNS or proxy settings if both attempts fail.[/dim]")
        return
    ...
    for number, item in enumerate(entries, 1):
        table.add_row(str(number), str(item.get("title", ""))[:65], str(item.get("channel") or item.get("uploader") or "")[:30])
    console.print(table)
...
def main() -> None:
    ...
    while True:
        ...
        for number, action in (("1", "🔗 Download URL"), ("2", "📋 Clipboard"), ("3", "🔎 Search YouTube"), ("4", "📚 History"), ("5", "⚙️ Settings"), ("6", "❌ Exit")):
